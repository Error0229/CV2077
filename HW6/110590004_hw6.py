    ...
    def __init__(self):
        ...
        for i in range(1, 4):
            ...
            for row in range(self.origin_images[i - 1].shape[0]):
                for col in range(self.origin_images[i - 1].shape[1]):
                    r, g, b = self.origin_images[i - 1][row][col].astype(np.float64)
        ...
        def P1():
            for i in range(1, 4):
                ...
                gaussian = gaussian_filter(self.images[i - 1], 5, 0.8)
                ...
                double_thresh = double_thresholding(non_max, 80, 190)
                save_img(double_thresh, f'./debug/img{i}_double_thresh.jpg')
                print('Edge tracking by hysteresis')
                edge_tracking = edge_tracking_by_hysteresis(deepcopy(double_thresh))
