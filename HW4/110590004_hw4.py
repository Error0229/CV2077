...
class Q:
    def __init__(self):
        ...
        for i in range(1, 4):
            img = np.zeros(
                (self.origin_images[i - 1].shape[0], self.origin_images[i - 1].shape[1], 3))
        ...
        def P1():
            ...
            def water_shed(image, id):
                ...
                #         rs[row][col] = 0.299 * image[row][col][2] + 0.587 * \
                #             image[row][col][1] + 0.114 * image[row][col][0]
                # import os
                # if not os.path.exists("./debug"):
                #     os.makedirs("./debug")
                # cv2.imwrite("./debug/img" + str(id) + "_q1_gray.png", rs)
                # image = deepcopy(rs)
                seeds = []
                pq = PriorityQueue()
                ...
                def priority(x, y, image, group_mean_color):
                    ...
                    # Compute mean color of the neighbors
                    mean_color = np.mean(colors, axis=0)
                    # Sum of variances of RGB channels
                            ...
                            for row in range(width):
                                for col in range(height):
                                    if a[row][col] == -1:
                                        ...
                                        frame[row][col] = (
                                            frame[row][col] * 0.3 + np.array(lm[a[row][col]]) * 0.7).astype(np.uint8)
                            frames.append(frame)
                            bar()
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    out = cv2.VideoWriter(
                        "./images/img" + str(i) + "_q1-3.mp4", fourcc, 3, (height, width))
                    print("writing video")
                    for frame in frames:
                        out.write(frame)
                    out.release()
                ...
                for row in range(width):
                    for col in range(height):
                        if land[row][col] <= 0:
                            ...
                            origin[row][col] = (
                                origin[row][col] * 0.3 + np.array(lm[land[row][col]]) * 0.7).astype(np.uint8)
